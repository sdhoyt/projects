...
Because we used a logarithmic transform for both the predictor and response variables, we **cannot** simply infer that for each 1 unit increase in the population density, we expect to see, on average, a `r round(coef$estimate, 2)` $\pm$ `r round(coef$estimate - coef$conf.low, 2)` increase in station density. Instead, taking the logarithmic transforms into account, we can infer from this model that for every 1% increase in the population density of a state, we can expect, on average, a `r round(coef$estimate, 2)`% $\pm$ `r round(coef$estimate - coef$conf.low, 2)`% increase in the station density for that state ([UVA Library StatLab](https://data.library.virginia.edu/interpreting-log-transformations-in-a-linear-model/))[^1].

[^1]: Because both the predictor and response variable have undergone a log tranformation, we start with the following equation for our linear model: $$log(y) = log(x)\beta_1 + \beta_0$$ where $y$ is the response variable, $x$ is the predictor variable, $\beta_1$ is the slope of our regression line and $\beta_0$ is our y intercept. Using basic log rules, we can manipulate the equation: $$y = 10^{log(x) \cdot \beta_1} \cdot 10^{\beta_0}$$ $$=10^{log(x^{\beta_1})} \cdot 10^{\beta_0}$$ $$y =x^{\beta_1} \cdot 10^{\beta_0}$$ From here, focusing on the relationship between $x$ and $y$, we can see that for every 1% increase in the predictor variable (since multiplying a value by $1.01 = 101\%$ means increasing the value by $1\%$), we get a $\beta_1\%$ increase in the response variable $$y = (1.01)^{1.04} = 1.010402 = 101.04\%$$ In other words, when the predictor variable increases by 1%, the response variable increases by the slope $\beta_1$ as a perentage value ($\beta_1\%$) or in this case, $1.04\%$.
